<script lang="ts">
  // A single inline-SVG icon set, keyed by name. Icons are decorative by
  // default (aria-hidden) since they are paired with text labels; pass a `title`
  // to give a standalone icon an accessible name.
  export let name: string;
  export let size = 18;
  export let title: string | undefined = undefined;

  // Static, trusted SVG inner markup — never user input. Stroke icons inherit
  ...
  const paths: Record<string, string> = {
    ...
    timeline: '<path d="M22 12h-4l-3 9L9 3l-3 9H2"/>',
    ...
    ssh: '<polyline points="4 17 10 11 4 5"/><line x1="12" y1="19" x2="20" y2="19"/>',
    tls: '<rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/>',
    shield:
      '<path d="M20 13c0 5-3.5 7.5-7.66 8.95a1 1 0 0 1-.67-.01C7.5 20.5 4 18 4 13V6a1 1 0 0 1 1-1c2 0 4.5-1.2 6.24-2.72a1.17 1.17 0 0 1 1.52 0C14.51 3.81 17 5 19 5a1 1 0 0 1 1 1z"/>',
...
<svg
  ...
  aria-label={title}
>
  <!-- eslint-disable-next-line svelte/no-at-html-tags -- static, trusted icon markup; never user input -->
  {@html markup}
</svg>
